...
def test_collect_simulation_stats_is_deterministic() -> None:
    ...
    assert first.event_records_json_bytes > 0
    assert first.rumor_archive_json_bytes > 0
    assert first.save_json_bytes > first.event_records_json_bytes
...
def test_format_stats_includes_required_counters() -> None:
    ...
    assert "event_records_json_bytes: " in text
    assert "rumor_archive_json_bytes: " in text
    assert "save_json_bytes: " in text
...
def test_main_prints_deterministic_json(capsys) -> None:
    ...
    assert set(data) >= {
        ...
        "alive",
        "total_months",
        "event_records_json_bytes",
        "rumor_archive_json_bytes",
        "save_json_bytes",
    }
