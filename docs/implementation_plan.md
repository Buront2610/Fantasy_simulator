...
| region の richer local semantics（門、市場、掲示板、河川、事故地点、痕跡の詳細化） | first pass 実装済み（site seed tags / terrain biome / runtime memory / route closure を category / tag / priority 付き局所手がかりへ統合） | 後続拡張 |
...
| Map UI | マップ画面改善 | active mainline。route state、current control、world-change traces、local decision cues を atlas / region / detail へ読みやすく統合し、局所手がかりカテゴリから地点を探せる導線まで追加済み |
