...
def _dead_leader_result(draft: AdventureDraftWorld, character: Any) -> AdventureStepResult:
    ...
    character.add_history(tr(
        ...
        detail=tr("detail_adventure_died", name=character.name, destination=draft.location_name(run.destination)),
    ))
    return step_fact_result(run, "adventure_death", "summary_adventure_died",
                            {"name": character.name, "destination": draft.location_name(run.destination)},
                            actor_id=character.char_id, severity=5)
