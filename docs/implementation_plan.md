...
@dataclass
class LocationState:
    id: str               # 不変の識別子 例: "aethoria_capital"
...
```
ルール: id = canonical_name を小文字にし、空白・特殊文字をアンダースコアで置換した slug
例:
  "Aethoria Capital"  → "aethoria_capital"
  "Frostpeak Summit"  → "frostpeak_summit"
  "The Grey Pass"     → "the_grey_pass"
  "Goblin Warrens"    → "goblin_warrens"
  "Sunken Ruins"      → "sunken_ruins"
```
...
DEFAULT_LOCATIONS = [
    ("frostpeak_summit",   "Frostpeak Summit",   "A jagged mountain crowned with eternal ice.",                  "mountain", 0, 0),
    ("the_grey_pass",      "The Grey Pass",       "A treacherous alpine pass haunted by wind spirits.",           "mountain", 1, 0),
    ("skyveil_monastery",  "Skyveil Monastery",   "A cliffside monastery where monks study the ley-lines.",       "village",  2, 0),
    ("ironvein_mine",      "Ironvein Mine",        "A deep mine rich in enchanted ore — and old curses.",          "dungeon",  3, 0),
    ("stormwatch_keep",    "Stormwatch Keep",      "A fortress overlooking the northern sea.",                     "mountain", 4, 0),
    ("thornwood",          "Thornwood",            "A dense forest that hums with restless magic.",                "forest",   0, 1),
    ("ashenvale",          "Ashenvale",            "Charred woodland recovering from a decade-old wildfire.",      "forest",   1, 1),
    ("silverbrook",        "Silverbrook",          "A prosperous trading town built on a swift silver river.",     "city",     2, 1),
    ("goblin_warrens",     "Goblin Warrens",       "A network of tunnels teeming with mischievous creatures.",     "dungeon",  3, 1),
    ("eastwatch_tower",    "Eastwatch Tower",      "A lone watchtower staffed by a rotating ranger garrison.",     "village",  4, 1),
    ("elderroot_forest",   "Elderroot Forest",     "An ancient forest whose trees remember the Cataclysm.",        "forest",   0, 2),
    ("millhaven",          "Millhaven",            "A quiet farming village known for its legendary apple wine.",  "village",  1, 2),
    ("aethoria_capital",   "Aethoria Capital",     "The grand capital — heart of trade, politics, and intrigue.",  "city",     2, 2),
    ("sunken_ruins",       "Sunken Ruins",         "Ruins of a pre-Cataclysm city, half-swallowed by the earth.", "dungeon",  3, 2),
    ("saltmarsh",          "Saltmarsh",            "A fishing village where sailors whisper of sea monsters.",     "village",  4, 2),
    ("dragonbone_ridge",   "Dragonbone Ridge",     "A ridge littered with the bones of ancient dragons.",          "mountain", 0, 3),
    ("dusty_crossroads",   "Dusty Crossroads",     "A well-worn junction where merchants rest and rumours spread.", "plains",  1, 3),
    ("hearthglow_town",    "Hearthglow Town",      "A warm, welcoming town renowned for its healers' guild.",      "city",     2, 3),
    ("mirefen_swamp",      "Mirefen Swamp",        "A murky swamp hiding both treasure and terrible dangers.",     "dungeon",  3, 3),
    ("dawnport",           "Dawnport",             "A busy harbour city that never truly sleeps.",                 "city",     4, 3),
    ("sunbaked_plains",    "Sunbaked Plains",      "Vast golden plains scorched by an unrelenting sun.",           "plains",   0, 4),
    ("sandstone_outpost",  "Sandstone Outpost",    "A small desert outpost at the edge of the known world.",       "village",  1, 4),
    ("the_verdant_vale",   "The Verdant Vale",     "A lush valley sheltered from harsh winds — a true paradise.", "village",  2, 4),
    ("obsidian_crater",    "Obsidian Crater",      "A massive crater from the Cataclysm, still faintly glowing.", "dungeon",  3, 4),
    ("coral_cove",         "Coral Cove",           "A hidden cove home to a secretive community of sea-mages.",    "city",     4, 4),
]
```
...
| 値の型 | 地点名（例: `"Aethoria Capital"`） | 地点 ID（例: `"aethoria_capital"`） |
| 参照先 | `World._location_name_index[name]` | `World.get_location_by_id(id)` |
| 互換プロパティ | なし | `character.location`（廃止予定）→ `LocationState.canonical_name` を返す |

### 4.2 互換ラッパーの扱い

**原則: 互換ラッパーは限定的・一時的にのみ設ける。**

- PR-3 での一括置換完了後、`character.location`（str プロパティ）は `DeprecationWarning` を出しながら `canonical_name` を返す互換プロパティとして残す。
- Phase 1 完了（PR-3 マージ）後 1 PR 以内に互換プロパティを削除する PR（PR-3b）を出す。
- **互換プロパティを新規コードで使ってはならない。** レビューで差し戻す。

```python
# character.py — 互換ラッパー（PR-3 で追加、PR-3b で削除）
@property
def location(self) -> str:
    """非推奨: location_id を使用してください。"""
    import warnings
    warnings.warn(
        "Character.location is deprecated; use location_id instead.",
        DeprecationWarning,
        stacklevel=2,
    )
    if self._world_ref is not None:
        loc = self._world_ref.get_location_by_id(self.location_id)
        if loc is not None:
            return loc.canonical_name
    return self.location_id  # フォールバック: id をそのまま返す
```

> `_world_ref` は World への弱参照（`weakref.ref`）。World への参照が不要な場合は単に `return self.location_id` のみで可。方針は PR-3 レビュー時に決定する。
...
| v1 | `schema_version: 1` 追加 + `Character.job` → `adventure_job` リネーム（廃止された旧フィールド対応） | PR-2 |
...
> **設計書 §15 の migration 例は旧案（v1=schema_version, v2=LocationState, v3=Relationship, v4=dying）です。本計画書の上記テーブルを正式とします。** 主な差分は v2 で location 参照を一括修正する点と、v3 で `LocationState` + `WorldEventRecord` を同時導入する点です。
...
LOCATION_NAME_TO_ID: dict[str, str] = {
    "Frostpeak Summit":  "frostpeak_summit",
    "The Grey Pass":     "the_grey_pass",
    "Skyveil Monastery": "skyveil_monastery",
    "Ironvein Mine":     "ironvein_mine",
    "Stormwatch Keep":   "stormwatch_keep",
    "Thornwood":         "thornwood",
    "Ashenvale":         "ashenvale",
    "Silverbrook":       "silverbrook",
    "Goblin Warrens":    "goblin_warrens",
    "Eastwatch Tower":   "eastwatch_tower",
    "Elderroot Forest":  "elderroot_forest",
    "Millhaven":         "millhaven",
    "Aethoria Capital":  "aethoria_capital",
    "Sunken Ruins":      "sunken_ruins",
    "Saltmarsh":         "saltmarsh",
    "Dragonbone Ridge":  "dragonbone_ridge",
    "Dusty Crossroads":  "dusty_crossroads",
    "Hearthglow Town":   "hearthglow_town",
    "Mirefen Swamp":     "mirefen_swamp",
    "Dawnport":          "dawnport",
    "Sunbaked Plains":   "sunbaked_plains",
    "Sandstone Outpost": "sandstone_outpost",
    "The Verdant Vale":  "the_verdant_vale",
    "Obsidian Crater":   "obsidian_crater",
    "Coral Cove":        "coral_cove",
}


def _name_to_id(name: str) -> str:
    """name → id を対応表で解決。不明な場合は slug フォールバック。"""
    if name in LOCATION_NAME_TO_ID:
        return LOCATION_NAME_TO_ID[name]
    # フォールバック: slug 変換（小文字・空白→アンダースコア・特殊文字除去）
    import re
...
def migrate_v1_to_v2(data: dict) -> dict:
    ...
    for char_data in data.get("characters", []):
        old_location = char_data.pop("location", "aethoria_capital")
...
| `character.py` | `from_dict()` の `location=data.get("location", ...)` | `location_id=data.get("location_id", "aethoria_capital")` |
...
| `character_creator.py` | `location="Aethoria Capital"` デフォルト | `location_id="aethoria_capital"` に変更 |

### 6.3 置換の順序

1. `world_data.py`: `DEFAULT_LOCATIONS` を id 付き形式に更新
2. `world.py`: `LocationState` 定義、`_location_id_index`、`get_location_by_id()` 追加
3. `character.py`: `location_id` フィールド化、互換ラッパー追加
4. 上記以外の参照ファイル: 一括修正
...
1. `world_data.py`: `DEFAULT_LOCATIONS` を `(id, canonical_name, description, region_type, x, y)` 形式に更新
2. `world.py`:
   ...
   - `get_location_by_name(name: str)` を deprecated として残す（`_location_name_index` は読み込み用に維持）
   - `render_map()` に danger/safety_label/traffic 簡易表示追加（設計書 §18.2）
3. `character.py`:
   - `location: str` → `location_id: str`
   - 互換ラッパー `location` プロパティ（DeprecationWarning 付き）を追加
4. `character_creator.py`: `location="Aethoria Capital"` → `location_id="aethoria_capital"`
5. `events.py` / `adventure.py` / `screens.py`: `character.location` → `character.location_id` に一括置換
...
- migration v1→v2 のテスト（全 built-in 地点名が正しく id に変換される）
- 旧セーブ（`location: "Aethoria Capital"` 形式）の読み込みテスト

**マージ条件**: 全テスト通過、SI-1 〜 SI-3 のテストが存在、flake8 通過

---

### PR-3b: 互換ラッパー削除

**ゴール**: `character.location` 互換プロパティの完全削除  
**作業**:
- `character.py` から互換ラッパー `location` プロパティ削除
- 残存参照（あれば）を全修正
- テスト: `character.location` への直接アクセスが `AttributeError` になることを確認（または削除のみで OK）
...
| Phase 1c | PR-3 | LocationState・location_id 一括移行 | **SI-1〜SI-3 確立** |
| Phase 1d | PR-3b | 互換ラッパー削除 | SI 維持 |
...
| 旧セーブの location 名に typo や独自名が含まれる | slug フォールバックで対応。load 時に警告ログを出す |
| 互換ラッパー削除（PR-3b）が後回しになる | PR-3 マージ時に PR-3b を同時に作成し Draft PR として登録する |
