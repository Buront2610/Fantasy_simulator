...
from ..i18n import tr
from .models import MonthlyReport, WorldChangeReportLine, YearlyReport
...
def format_monthly_report(report: MonthlyReport) -> str:
    ...
    if report.character_entries:
        ...
        for character_entry in report.character_entries:
            lines.append(f"    {character_entry.name}")
            for event in character_entry.events:
    ...
    if report.rumor_entries:
        ...
        for rumor_entry in report.rumor_entries:
            reliability_label = tr(f"rumor_reliability_{rumor_entry.reliability}")
            lines.append(f"    - {rumor_entry.description} ({reliability_label})")
