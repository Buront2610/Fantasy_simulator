...
| K0-7 save/migration policy | Started | v8 serializer/hydrator facade is split into serializer / hydrator / terrain overlay helpers without changing save shape; war plus era/civilization APIs round-trip through canonical records, restore projection/report views, and preserve derived location-state pressure without durable era runtime fields; new durable runtime fields still require ADR + conflict policy + migration tests |
