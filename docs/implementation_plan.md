...
| PR | 内容 | この文書での位置づけ |
|---|---|---|
| PR-0 | 文書正本の確定 + README 同期 | ✅ 本 PR で完了。以後の前提条件 |
| PR-A | 最小 package 化 | 次の実装着手点 |
| PR-B | 月次エンジン化 | package 化の次 |
| PR-C | `simulator.py` 分割 + event store 正規化完了 | Phase B 後 |
