...
**最終更新**: 2026-03-24 (main 反映: PR-G1/G2 実装済み)  
...
- ~~真の月次進行は未完成~~ → PR-B で月次ループ基盤を導入済み。フル月次ループ（NPC 行動・依頼生成等）は後続 PR で段階的に拡張
- ~~`simulator.py` の責務集中が強い~~ → PR-C で `simulation/` サブパッケージへ mixin ベース分割済み
- イベントストアは方針上 `event_records` を正規とし `history` / `event_log` を補助層と位置づけたが、永続化上はまだ三重保持が残る（後続 PR で段階的に縮退予定）
...
- `NarrativeContext` 主体の文脈依存叙述は未完成であり、relation tags / memorial / reports / world memory との本格接続は PR-I 以降の課題である
...
現段階は「PR-G2 まで到達した後の観測体験仕上げ段階」である。

terrain / site / route / world memory を読むための基礎データと三層観測 UI はすでに揃っている。
ただし実装密度には偏りがあり、atlas / detail はかなり前進した一方、
region はまだ「近傍切り出し + route 線 + landmarks」の色が強い。

したがって次の公式着手対象は **PR-H（region 観測強化 + 軽量 Rich シェル）** とする。
主線は region map の意味論強化であり、並行して固定レイアウト、強調、余白、操作導線などの
薄い Rich 化を入れて観測体験を底上げする。

一方、PR-G3 / PR-G4 に相当する worldgen PoC や外部比較実験は、
...
8. PR-H（region 観測強化 + 軽量 Rich シェル）← **次はここ**
9. PR-I（`NarrativeContext` 拡張）
10. PR-G3 / PR-G4 相当の worldgen PoC / 外部比較は並行任意の技術検証として扱う
...
| 可変ワールド対応 + 地形 PoC + 観測UI初版 | 一部実装済み（PR-G1/G2 完了、G3/G4 未着手） | PR-G |
| region 観測強化 + 軽量 Rich シェル | 一部実装済み（PR-H1） | PR-H |
| `NarrativeContext` 拡張 | 未着手 | PR-I |
...
次に着手すべき実装は **PR-H（region 観測強化 + 軽量 Rich シェル）** である。

その理由は、現在の main ブランチでは atlas / detail はかなり前進した一方、
region がまだ「判断できる局所地図」には届いていないからである。
主線は region の意味論強化とし、固定レイアウト、意味色、入力補助などの
薄い Rich 化はそれを検証しやすくする補助線として並行する。

なお、PR-G3 / PR-G4 相当の worldgen PoC と外部比較実験は、
mainline の UX 改善を止めない範囲で並行任意の技術検証として扱う。
`NarrativeContext` 拡張は、PR-H 完了後に PR-I として着手する。
その際、Aethoria の lore / race / job / location 定義を唯一の固定正史として扱わず、
外部設定 bundle として後から差し替え・追加できる土台も同時に整える。
...
- `terrain.py` 導入: `TerrainCell`, `TerrainMap`, `Site`, `RouteEdge`, `AtlasLayout`
- `World(width, height)` による可変サイズ world 基盤
- `World.terrain_map` / `sites` / `routes` / `atlas_layout` の永続化
- migration v5 → v6 → v7: terrain/site/route と atlas layout の後方互換追加
- `ui/map_renderer.py`: terrain-aware `MapRenderInfo`、world overview / region / detail 描画
- `ui/atlas_renderer.py`: atlas overview、compact、minimal の各表示モード
- `ui/screens.py`: atlas ベース world map navigation と地点履歴 UI の接続
- presenter / view-model 層の最小導入
- テスト: terrain / map observation / migration / variable-size world を追加
...
9. PR-H: region 観測強化 + 軽量 Rich シェル ← **次はここ**
10. PR-I: `NarrativeContext` 拡張
11. PR-J: 世界観設定整理と初期 Setting Bundle 構築
12. PR-K: 動的世界変化（war / renaming / terrain mutation / era shift / civilization drift）
13. PR-G3 / PR-G4: worldgen PoC / 比較実験（並行任意）
...
次の公式着手対象は **PR-H（region 観測強化 + 軽量 Rich シェル）** である。
ここでは豪華な UI を先に作るのではなく、まず region を
「導線・危険・履歴を判断できる局所地図」へ引き上げる。
そのうえで、固定レイアウト、意味色、入力補助などの薄い Rich 化により、
観測体験を検証しやすくし、読みやすさを底上げする。

その後、PR-I（`NarrativeContext` 拡張）で、
広い世界と蓄積された履歴を文脈依存の叙述へ接続する。
この段階では同時に、基礎世界設定を `content/world_data.py` の単一ハードコードから外し、
外部データとして注入できる土台を整える。
...
| PR-H | region 観測強化 + 軽量 Rich シェル | 観測体験の mainline 強化 |
| PR-I | `NarrativeContext` 拡張 + 基礎世界設定の外部化土台 | 群像劇の叙述強化と setting bundle 化 |
| PR-J | 世界観設定整理と初期 Setting Bundle 構築 | 最初の正式 world setting を構築 |
| PR-K | 動的世界変化 | war / renaming / terrain mutation / era shift / civilization drift |
