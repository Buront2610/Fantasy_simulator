...
def test_readme_and_plan_agree_on_next_step() -> None:
    ...
    assert "The next mainline focus is integrated design v1.0" in README_TEXT
    assert "現在進めるべき実装は **統合設計 v1.0**" in PLAN_TEXT
    assert "PR-K は完了済み" in PLAN_TEXT
    assert "integrated design v1.0 as active mainline" in RISK_REGISTER_TEXT
...
def test_ui_plan_does_not_conflict_with_implementation_plan_next_step() -> None:
    ...
    assert "統合設計 v1.0 ← **active mainline**" in PLAN_TEXT
