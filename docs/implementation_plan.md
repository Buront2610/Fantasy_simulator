...
| 段階 | 実装・完了判定 | 状態 |
|---|---|---|
| M0 信頼性 | 戦闘・健康・冒険事実、保存後の継続、必要履歴の保持 | 戦闘決着に続き、実被害に応じた冒険負傷と帰還時の健康参照を実装 |
...
これらのPRで設計全体または M0 全体を完了扱いにしない。

第二PR（§16.3/16.4/16.5）は、冒険の実被害ゼロなら新たな負傷を付与せず、
危険な遭遇でも勝者に敗者向けの瀕死補正を付けない。帰還は全参加者の現在の健康を読み、
救助・治療後に古い冒険側の負傷を再適用しない。冒険死亡は既存の共通後処理を通す。
無傷の遭遇を発見と区別し、負傷・遭遇のイベント主体を実際の仲間に一致させる。
実際の救助から帰還、正式な保存/読込、非リーダーの被害記録、32固定シードの防御技能比較で検証する。
この段階の状態は本節を正本とし、README等には個々の段階の進捗を複製しない。

次は型付き冒険事実と原子適用の接続。同日救助期限、健康変更入口の統一、
移動中の健康期限、資産継承まで含む死亡処理、言語史と統合シナリオは残作業。
...
- ~~UI 層と domain 層の分離が不十分~~ → PR-D で `ui/map_renderer.py` + `ui/ui_context.py` 導入。`screens.py` / `main.py` / `character_creator/` の全 I/O を `InputBackend` / `RenderBackend` 経由に移行完了。ゼロ stdout 漏洩テストで差し替え可能性を証明済み。将来の Rich / Textual 導入基盤が整った
...
- ~~`CharacterNarrativeState` / `CharacterAbilities` / `Relationship` の本格構造化は未完了~~
  → TD-1〜TD-4 で domain value object へ分離済み
- ~~`WorldEventRecord.tags` は未導入~~ → PR-B で導入済み。`summary_key` も TD-1〜TD-4 で導入済み
...
TD-1〜TD-4 の負債解消では、`World.event_records` 正規化、`SettingBundle` の実データ化、
`world.py` / `events/` の責務圧縮、guardrail / harness / 文書同期の再強化を完了した。
...
TD-1〜TD-4 の進行中に、観測 UI と authoring まわりでは次の debt payoff を先行で取り込んだ。

- rumor / observation UX は「表示されているだけ」ではなく inspectable な詳細断面を持つこと
- bundle authoring / swap review は region breakdown, route breakdown, language coverage を純関数で点検できること
- type-gate scaffolding は pyproject 上の設定として先に置き、後続で quality gate に段階接続できること
...
11. ~~TD-1〜TD-4: 負債解消（canonical event store / SettingBundle 外部化 / 責務分離 / guardrail 強化）~~ ✅ 完了
...
war open/close、terrain-cell mutation、headless era/civilization projection、
natural world-change generation、SettingBundle rule/baseline input の guardrail slices は
completed として扱い、今後のマップ画面改善や後続拡張で退行させない。
### 3.4 現在地の追跡用サマリー
...
| 項目 | 状態 | 次の参照 PR |
|---|---|---|
| `CharacterNarrativeState` / `CharacterAbilities` / `Relationship` の本格構造化 | 実装済み | TD-1〜TD-4 |
...
| region の richer local semantics（門、市場、掲示板、河川、事故地点、痕跡の詳細化） | first pass 実装済み（site seed tags / terrain biome / runtime memory / route closure を category / tag / priority 付き局所手がかりへ統合し、地点詳細から地点履歴へ進める follow-up 導線、site type 別の局所AA、地域図の拡大 route sketch も追加） | 後続拡張 |
...
> Current status: the package split has moved beyond this first-stage sketch.
> The current implementation keeps public compatibility facades at
> `character.py`, `world.py`, `events/`, `adventure/`, `terrain/`, `reports/`,
> and `rumor/`, while narrower implementation surfaces live under
> `character_model/`, `combat_system/`, `world_map/`, and the split `world_*`
> packages. Treat the tree below as historical Phase A planning context, not as
...
> Current note: the repository chose an incremental domain-package split rather
> than the speculative `core/` / `mechanics/` tree below. New work should follow
> the current package names documented in `README.md`, `AGENTS.md`, and
...
- `character_creator/` の対話入力パスも `UIContext` 経由に移行
...
- culture / faction は、まず既存 language community・site seed・location state に対応する
  flat authoring list として保守的に seed し、詳細な政治史や conflict state は PR-K に送る
...
> 現時点の Aethoria bundle には、既存 lore・languages・site seeds に沿った culture / faction の
> first pass を追加済みであり、構造化された関係性・支配・戦争状態は PR-K の状態機械化で扱う。
...
> PR-K の準備として、world-change event adapter の contract は先行して固めた。
> route block/reopen record は `render_params` の endpoint IDs と `location:*` tags で
> 両端地点から visibility / report query できることを保証し、表示文は
> `summary_key` / `render_params` / `description` fallback で扱う。
> PR-K は完了済みであり、contract / guardrail は後続のマップ画面改善で維持する基盤である。
> completion は、Command → validation → state machine → domain event →
> `WorldEventRecord` → `ChangeSet` → reducer → projection/view model → UI/report → tests の
> 縦切りで閉じる。

#### PR-K K0 completion checklist

| Slice | Status | Completion target |
|---|---|---|
| K0-2 boundary / contract / invariant | Completed | architecture boundaries, event contracts, legacy-read policy, and property invariants are standard quality-gate targets; strict gate also covers them before PR-K behavior broadens |
| K0-3 characterization / golden masters | Completed | seeded characterization plus report/map golden coverage now proves current behavior before broadening dynamic changes; the PR-K golden contract covers route block, location rename, war declaration, occupation, terrain mutation, era shift, civilization drift, monthly cards, dashboard state, report projections, map overlays, and save/load stability |
| K0-4 typed ID ratchet | Completed | location / route / faction / event / era / culture IDs expand without breaking legacy string-load compatibility; PR-K command boundaries share nominal-ID normalization helpers for location, route, faction, event, era, and terrain-linked IDs before domain events become records. Terrain mutation commands accept optional typed `TerrainCellId` values and reject IDs that do not match the requested coordinates, and culture/faction authoring inspection keys now use the same shared slug-ID normalization while preserving legacy display-name storage |
| K0-5 route / rename / occupation | Completed | route block/reopen, including configured natural route disruptions/reopenings during simulation, natural official renames, and occupation state-machine slices are visible through canonical records, projections, monthly/yearly reports, UI report cards, region/atlas map overlays, endpoint/location-state pressure, live traces, save/load contracts, and focused tests; currently blocked routes reach the observer dashboard from route-status projections, current occupation/control state reaches it from canonical war-map projections, recent concrete world-change entries reach it from world-change report projections, and the PR-K golden contract pins these surfaces together |
| K0-6 war / terrain / era / civilization projection | Completed | war declarations and war endings derive active faction-war state from canonical records, apply deterministic location-state pressure, remain map/report/dashboard/UI-card visible, and round-trip through save/load without durable war runtime fields; dashboard also surfaces current route closures, current occupation/control state, current era/civilization phase, and recent concrete world-change entries from canonical projections; simulation can now spend the natural world-change budget on faction-war openings/endings, war-driven location-control shifts, location-linked terrain mutations, natural era shifts, civilization drift, and route changes, with same-slot fallback when the first selected generator cannot produce a valid change; natural world-change records also flow into pending notifications and can stop auto-advance with world-change pause context/recommendations; location-linked terrain mutations, era shifts, and civilization score drift also apply deterministic location-state pressure while remaining canonical-record driven; transient era runtime fields are explicitly non-durable under the K0 save policy |
| K0-7 save/migration policy | Completed | v8 serializer/hydrator facade is split into serializer / hydrator / terrain overlay helpers without changing save shape; war plus era/civilization APIs round-trip through canonical records, restore projection/report views, and preserve derived location-state pressure without durable era runtime fields; hydrator now explicitly discards stale experimental era runtime snapshot fields, canonical records win conflicts, and snapshot-only era/civilization payloads remain unknown until a future durable schema adds ADR + conflict policy + migration tests |

Technical-review support work is also moving alongside PR-K: ordinary activity,
journey, lifecycle, health, relationship, and combat event replay now have
cross-locale semantic metadata coverage across their main branches, and
auto-pause reason selection now derives reason, priority, and context from one
candidate path. Notification thresholds also consume canonical actor ids plus
semantic actor-id metadata, so replayed records can still notify watched
actors. The canonical event write path now keeps only the record-id duplicate
set current during append and defers the heavier location/actor/year/kind
indexes to read-side `ensure_current()` rebuilds, avoiding full event-signature
work on every canonical write.
...
canonical event store、bundle authoring source、責務境界、guardrail のねじれが閉じたため、
次の世界観拡張を安全に進める条件が整ったからである。

region map 自体の richer local semantics
（門、市場、掲示板、河川、事故地点、先行パーティの痕跡の詳細化など）は、
ここからの公式な後続拡張として扱う。

なお、PR-G3 / PR-G4 相当の worldgen PoC と外部比較実験は、
mainline の UX 改善を止めない範囲で並行任意の技術検証として扱う。
PR-J で Aethoria の正式 authoring を完了し、PR-K で動的世界変化を state machine 化した。
現在地は、world-change event contracts、architecture fitness guard、
property-based invariant tests、standard / strict quality gate 接続を前提に、マップ画面へ現在世界状態をより読みやすく出す段階である。
...
「何をシミュレートしている世界なのか」が定義された状態へ進めた。

その後の PR-K では、
戦争、地名変化、地形変化、時代変化、文明変化といった
「歴史として世界が変わる要素」を正式に state machine 化した。
...
| SI-9 | `WorldEventRecord.month/day` は下限のみ正規化（`>=1`）し、上限は calendar-aware 層で扱う | `world_event/models.py`, `tests/test_reports.py`, `tests/test_event_models.py` |
...
| v8 | `setting_bundle` と historical calendar change 保存 | 実装済み |

補足:

- `migrations.py` の `CURRENT_VERSION` は 9
...
| PR-D | UI 責務分離 | ✅ 完了。`ui/map_renderer.py` 導入（`MapCellInfo` / `MapRenderInfo` 中間表現 + `build_map_info()` / `render_map_ascii()`）。`ui/ui_context.py` 導入（`UIContext` 依存コンテナ）。`screens.py` / `main.py` / `character_creator/` の全 I/O を `InputBackend` + `RenderBackend` 経由に移行。直接 `print()` / `input()` を排除し、差し替え可能な UI 基盤を確立。統合テスト（ゼロ stdout 漏洩テスト含む）で証明済み |
...
| TD-1〜TD-4 | canonical event store / SettingBundle 外部化 / 責務分離 / guardrail 強化 | ✅ 完了。PR-J と PR-K の前提整理として閉じた |
| PR-J | 世界観設定整理と初期 Setting Bundle 構築 | ✅ 完了（culture / faction / language coverage / glossary / route seeds の初期 authoring） |
| PR-K | 動的世界変化 | ✅ 完了。route/rename/occupation、war open/close、terrain mutation、era shift、civilization drift、natural generation、SettingBundle rule/baseline input、projection/report/dashboard/map visibility、save policy、typed-ID ratchet、golden/guardrail coverage まで完了 |
| Map UI | マップ画面改善 | active mainline。route state、current control、world-change traces、local decision cues を atlas / region / detail へ読みやすく統合し、局所手がかりカテゴリから地点を探せる導線、地点詳細から地点履歴へ進める follow-up 導線、地点詳細の局所AA、地域図の拡大 terrain/site overlay と route sketch、dashboard follow-up から人物/地点観測へ進める導線、地点詳細での地名由来表示、都市AA拡大、地点詳細の局所 route sketch、地域 route sketch の都市街区化と道路接続表現、EN/JA 幅崩れ snapshot harness、canonical record 起点の通知/レポート/dashboard 表示ガード、主要画面 snapshot-style テストまで追加済み |
