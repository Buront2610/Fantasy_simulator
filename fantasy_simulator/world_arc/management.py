    ...
    for raw_record in records:
        ...
        if record.kind == "war_declared":
            ...
            if not aggressor or not target or aggressor == target:
                continue
            declared_arc = _war_arc_from_declaration_record(record)
            arcs.append(declared_arc)
            active_by_pair[war_pair(aggressor, target)] = declared_arc
            continue
        if record.kind == "war_ended":
            aggressor = str(params.get("aggressor_faction_id", ""))
            target = str(params.get("target_faction_id", ""))
            ended_arc = active_by_pair.pop(war_pair(aggressor, target), None)
            if ended_arc is None:
                continue
            ended_arc.phase = "resolved"
            ended_arc.metadata["resolution_record_id"] = record.record_id
            ended_arc.touch(year=record.year, month=record.month, day=record.day, record_id=record.record_id)
            continue
        matched_arc = _record_arc_match(record, active_by_pair.values())
        if matched_arc is not None:
            matched_arc.touch(year=record.year, month=record.month, day=record.day, record_id=record.record_id)
    return arcs
