...
class AdventureSerialization:
    ...
    def to_dict(run: AdventureRunLike) -> Dict[str, Any]:
        return {
            ...
            "combat_logs": list(run.combat_logs),
            "related_event_ids": list(run.related_event_ids),
            ...
            "itinerary": run.itinerary.to_dict() if run.itinerary is not None else None,
            "objective": run.objective.to_dict() if run.objective is not None else None,
        }
    ...
    def from_dict(run_cls: Type[TAdventureRun], choice_cls: Type[Any], data: Dict[str, Any]) -> TAdventureRun:
        ...
        run = run_factory(
            ...
            combat_logs=list(data.get("combat_logs", [])),
            related_event_ids=list(data.get("related_event_ids", [])),
            ...
            itinerary=AdventureItinerary.from_dict(data["itinerary"]) if data.get("itinerary") is not None else None,
            objective=AdventureObjective.from_dict(data["objective"]) if data.get("objective") is not None else None,
        )
