...
したがって、次の公式着手対象は **PR-H2（薄い Rich シェル）** とする。
ここでは固定レイアウト、見出し、余白、強調、操作導線、意味色の最小導入により、
既存の観測 UI を「文字列を順に流す CLI」から一段引き上げることを主題とする。
...
| 薄い Rich シェル（固定レイアウト、見出し、余白、強調、意味色、操作導線） | 未着手 | PR-H2 |
...
次に着手すべき実装は **PR-H2（薄い Rich シェル）** である。

その理由は、現在の main ブランチでは atlas / detail / region の観測意味論はかなり前進した一方で、
固定レイアウト、意味色、余白、見出し、差分再描画、入力補助といった
「リッチな CLI の本体」はまだ未導入だからである。
したがって次段では、既存の観測 UI を壊さずに、
薄い Rich 化によって表示密度と操作体験を底上げする。
...
次の公式着手対象は **PR-H2（軽量 Rich シェル）** である。
固定レイアウト、意味色、入力補助などの薄い Rich 化により、
...
| PR-H2 | 薄い Rich シェル | 次の mainline 強化対象 |
