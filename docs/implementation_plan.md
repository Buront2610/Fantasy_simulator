...
| 真の月次進行 | 実装済み | PR-B |
| `simulator.py` 分割 + イベントストア一本化の完了 | 未着手 | PR-C |
| UI 責務分離 | 未着手 | PR-D |
| パーティ冒険 | 未着手 | PR-E |
...
| PR-B | 月次エンジン化 | ✅ 完了。`_run_month()` 導入、`advance_months()` 追加、`SIMULATION_DENSITY` 最小版導入、`WorldEventRecord.tags` 追加 |
| PR-C | `simulator.py` 分割 + event store 正規化完了 | 次の実装着手点 |
| PR-D | UI 責務分離 | Phase C 後 |
