...
**最終更新**: 2026-06-03 (main 反映: PR-K は active mainline。K0 guardrail slices は completed)
...
`world.py` / `events.py` の責務圧縮、guardrail / harness / 文書同期の再強化を完了した。
PR-J の初期 `SettingBundle` authoring も完了済みである。今後はこの guardrail を維持しながら、
active mainline milestone である PR-K の動的世界変化を前へ進める。PR-K は完全未着手ではなく、
route block/reopen、location rename、occupation/control、terrain-cell mutation、headless era/civilization
projection の K0 guardrail slice は完了済みである。ただし、これは PR-K behavior fully implemented を意味しない。
...
13. PR-K: 動的世界変化（war / renaming / terrain mutation / era shift / civilization drift） ← **active mainline**
    - K0 guardrail slices は completed: route block/reopen、location rename、occupation/control、terrain-cell mutation、headless era/civilization projection、characterization / golden masters、typed-ID ratchet、save/persistence policy、user-visible world-change integration
    - 残件: K0 のガードレールを維持しながら、PR-K 本体の世界変化 behavior をより広く・動的に拡張する
14. PR-G3 / PR-G4 相当の worldgen PoC / 外部比較は並行任意の技術検証として扱う

PR-K は未着手ではない。route block/reopen、location rename、occupation/control、
terrain-cell mutation、headless era/civilization projection の K0 guardrail slices は
完了済みである。K0 characterization / golden masters、typed-ID ratchet、
save/persistence policy、user-visible world-change integration も K0 完了条件として閉じたため、
以後はこのガードレールを維持しながら PR-K 本体の世界変化 behavior を拡張すること。
### 3.4 現在地の追跡用サマリー
...
> 現在の PR-K は active mainline であり、contract / guardrail は先行 merge 済みの途中状態である。
> K0 completion は、Command → validation → state machine → domain event →
...
現在進めるべき実装は **PR-K: 動的世界変化** である。
PR-K は active mainline milestone であり、契約・guardrail は一部 merge 済み、残り slice を完了へ寄せる段階である。
...
mainline の UX 改善を止めない範囲で並行任意の技術検証として扱う。
PR-J で Aethoria の正式 authoring を完了したため、PR-K で動的世界変化を進めている。
ただし現在地は「PR-K 未着手」ではなく、world-change event contracts、architecture fitness guard、
property-based invariant tests、standard / strict quality gate 接続を前提に、K0 guardrail slice の完了条件を閉じた段階である。
...
14. PR-K: 動的世界変化（war / renaming / terrain mutation / era shift / civilization drift） ← **active mainline**
    - K0 guardrail slices は completed: route block/reopen、location rename、occupation/control、terrain-cell mutation、headless era/civilization projection、characterization / golden masters、typed-ID ratchet、save/persistence policy、user-visible world-change integration
    - 残件: K0 のガードレールを維持しながら、PR-K 本体の世界変化 behavior をより広く・動的に拡張する
...
実運用に耐える guardrail へ接続済みであり、次の公式着手対象は
**PR-K: 動的世界変化** である。
...
- `migrations.py` の `CURRENT_VERSION` は 8
...
| PR-K | 動的世界変化 | active mainline。K0 guardrail slices は completed。era/civilization runtime rules、初期 faction relationship、初期 site controlling faction は SettingBundle から解決できる入口を追加済み。war / renaming / terrain mutation / era shift / civilization drift を K0 のガードレール上でさらに拡張する |
