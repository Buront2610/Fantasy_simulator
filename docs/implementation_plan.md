...
**最終更新**: 2026-03-31 (main 反映: PR-H2 実装済み。PR-I は slice 1 として relation-aware memorial/alias と SettingBundle 最小土台に着手)  
...
したがって、次の公式着手対象は **PR-I（`NarrativeContext` 拡張）** とする。
ここでは relation tags / rumor / memorial / reports / world memory を文脈依存叙述へ接続し、
群像劇の文面が固定テンプレートに偏らないようにすることを主題とする。
...
10. PR-I（`NarrativeContext` 拡張）← **次はここ**
11. PR-G3 / PR-G4 相当の worldgen PoC / 外部比較は並行任意の技術検証として扱う
### 3.4 現在地の追跡用サマリー
...
| `NarrativeContext` 拡張 | 着手中（slice 1: relation-aware memorial/alias + bundle lore path） | PR-I |
...
> 2026-03-31 時点では slice 1 として、memorial / alias に relation tags を流し込み、
> `screen_world_lore()` を最小 `SettingBundle` 参照へ切り替える土台まで完了。
> reports / rumors との本格接続とテンプレート拡張は PR-I の後続 slice で継続する。
...
> 進捗メモ (2026-03-31): `WorldDefinition` / `SettingBundle` の最小 dataclass・JSON loader・
> `World.setting_bundle` 導線を導入し、world lore 画面は bundle 起点へ切り替えた。
> 次の slice では reports / rumors の narrative 入力とテンプレート選択を広げる。
...
次に着手すべき実装は **PR-I（`NarrativeContext` 拡張）** である。

その理由は、現在の main ブランチでは PR-H2 まで完了し、atlas / detail / region の観測意味論に加えて
固定レイアウト、意味色、余白、見出し、入力補助といった薄い Rich シェルも導入済みだからである。
したがって次段では、表示基盤の次の課題として、
蓄積された関係性・噂・記憶を文脈依存叙述へ接続する PR-I を進める。
...
mainline の UX 改善を止めない範囲で並行任意の技術検証として扱う。
`NarrativeContext` 拡張は、PR-H2 完了後に PR-I として着手する。
...
次の公式着手対象は **PR-I（`NarrativeContext` 拡張）** である。
PR-H2 で導入済みの薄い Rich シェルを土台に、
...
| PR-I | `NarrativeContext` 拡張 + 基礎世界設定の外部化土台 | 次の mainline 強化対象 |
