...
- `NarrativeContext` 主体の文脈依存叙述は未完成であり、relation tags / memorial / reports / world memory との本格接続は PR-H 以降の課題である
- 5×5 盤面から離脱した可変サイズ world、terrain / site / route 分離、worldgen PoC は未着手である
- 高密度 AA マップは未着手であり、現状の地図は依然として「簡易表示」段階に留まる

### 3.3 現在の判断

現段階は「PR-E / PR-F を実装した後の品質強化段階」であり、**次は PR-G（可変ワールド対応 + 地形 PoC + 観測UI初版）へ安全に進む段階**である。
...
7. PR-G（可変ワールド対応 + 地形 PoC + 観測UI初版）へ着手 ← **次はここ**
...
| 可変ワールド対応 + 地形 PoC + 観測UI初版 | 未着手 | PR-G |
...
PR-0 から PR-F まで完了し、world memory（live trace / memorial / alias）が実装済みである。次に着手すべき実装は **PR-G（可変ワールド対応 + 地形 PoC + 観測UI初版）** である。
...
7. PR-G: 可変ワールド対応 + 地形 PoC + 観測UI初版 ← **次はここ**
...
PR-0 から PR-F までが完了し、パーティ冒険と world memory（live trace / memorial / alias）まで導入済みである。**次の公式着手対象は PR-G（可変ワールド対応 + 地形 PoC + 観測UI初版）であり、その後 PR-H（`NarrativeContext` 拡張）へ進む。**
...
| PR-G | 可変ワールド対応 + 地形 PoC + 観測UI初版 | world representation と観測 UI の強化 |
