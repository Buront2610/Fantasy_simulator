...
**最終更新**: 2026-06-01 (main 反映: PR-J は完了。PR-K は active mainline で、K0 guardrail slice は started)
...
> 2026-06-01 時点では、route block/reopen、location rename、occupation/control、terrain-cell mutation、
> headless era/civilization projection の K0 executable guardrail slice が started である。
> 残る work は K0-2/K0-3 の characterization / golden master 補強、K0-4 typed-ID ratchet、
> K0-7 save/persistence policy gap の明文化、UI/report から観測できる world-change integration である。
...
mainline の UX 改善を止めない範囲で並行任意の技術検証として扱う。
PR-J で Aethoria の正式 authoring を完了したため、次は PR-K で動的世界変化へ入る。
...
| PR-K | 動的世界変化 | active mainline。K0 guardrail slice は started。残件は characterization / golden masters、typed-ID ratchet、save/persistence policy gaps、user-visible integration |
