...
- イベントストアは方針上 `event_records` を正規とし `history` / `event_log` を補助層と位置づけたが、永続化上はまだ三重保持が残る（PR-D 以降で段階的に縮退予定）
- ~~UI 層と domain 層の分離が不十分~~ → PR-D で `ui/map_renderer.py` 導入、`InputBackend` / `RenderBackend` 抽象化済み。`world.render_map()` は UI 層へ委譲。将来の Rich / Textual 導入基盤が整った
...
現段階は「派手な新機能を増やす段階」ではなく、**Phase E の本命機能を安全に積める構造へ整える段階**である。
...
| UI 責務分離 | 実装済み | PR-D |
| パーティ冒険 | 未着手 | PR-E |
| live trace / memorial / alias | 未着手 | PR-F |
| 高密度 AA マップ | 未着手 | PR-G |
...
PR-0 から PR-D まで完了し、構造整備フェーズは終了した。次に着手すべき実装は **PR-E（パーティ冒険 Phase 1）** である。

**理由**:

- PR-D により UI 責務分離が完了し、domain / UI / persistence の責務境界が明確になった
- Phase E 着手条件（月次進行が安定、UI / simulation / persistence の責務境界が見えている）を満たしている
- 構造整備が完了したため、ゲーム体験を向上させる本命機能に着手する段階にある
...
PR-0 から PR-D までの構造整備フェーズが完了し、package 化・月次化・イベント整理・UI 分離の基盤が整った。**今後は Phase E の本体機能（パーティ冒険）に着手できる段階にある。**
...
- `migrations.py` の `CURRENT_VERSION` は 3
...
| PR-D | UI 責務分離 | ✅ 完了。`ui/map_renderer.py` 導入（`MapCellInfo` / `MapRenderInfo` 中間表現 + `build_map_info()` / `render_map_ascii()`）。`world.render_map()` は後方互換ラッパーへ縮小し、描画ロジックを UI 層へ移動。`ui/input_backend.py`（`InputBackend` プロトコル + `StdInputBackend`）と `ui/render_backend.py`（`RenderBackend` プロトコル + `PrintRenderBackend`）で入出力抽象を導入。ロケール依存分岐は `_choose_key` で解消済み、旧 `_choose` は削除。`world.py` から表示幅ユーティリティを除去し `ui_helpers` に一本化。`MapCellInfo` は将来レンダラー向けに prosperity / mood / rumor_heat / road_condition を保持 |
| PR-E | パーティ冒険 | 構造整備後の本命機能 |
| PR-F | world memory | 長期記憶の導入 |
