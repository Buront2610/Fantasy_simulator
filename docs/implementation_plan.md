...
| K0-4 typed ID ratchet | Started | location / route / faction / event / era / culture IDs expand without breaking legacy string-load compatibility; PR-K command boundaries now share nominal-ID normalization helpers for location, route, faction, event, era, and terrain-linked IDs before domain events become records. Terrain mutation commands accept optional typed `TerrainCellId` values and reject IDs that do not match the requested coordinates |
